...
デフォルトのルールを作成するには、意思決定ノードを選択し、デフォルトではDetailsタブになっているため**Logicタブ**をクリックしてから右上に表示される＋ボタンをクリックし、一番下に表示されるDefault ruleを選択します。  
...
ルールの名前を**decline unemployed**として、Select the criteria for your ruleでcredit scoreを選択してから**create**をクリックしてください。
